...
static long adjust_time_zone(ptr dtvec, struct tm *tmxp, ptr given_tzoff) {
...
#ifdef WIN32
  {
    ...
    if (given_tzoff == Sfalse) {
      ...
      tzname = S_string(NULL, len);
      while (len--)
        Sstring_set(tzname, len, w_tzname[len]);
    }
  }
#else
...
# if defined(__linux__) || defined(SOLARIS)
    ...
    tzname = S_string(tzname[tmxp->tm_isdst], -1);
# else
    /* BSD variants add `tm_zone` in `struct tm`: */
    tzname = S_string(tmxp->tm_zone, -1);
# endif
  }
#endif
  ...
  INITVECTIT(dtvec, dtvec_tzname) = tzname;
