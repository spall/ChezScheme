...
(define (get-l4-index n)
  (fxand (fxsrl n (constant eq-bitset-lo-bits)) (constant eq-bitset-l4-mask)))

(define (eq-bitset-member? bm p)
  (let* ([n ($fxaddress p)]
         [a (get-l3 (get-l2 (get-l1 bm n) n) n)]
         [i (get-l4-index n)])
    (fxbit-set? (fxvector-ref a i) (fxand n (constant eq-bitset-lo-mask)))))
...
(define (eq-bitset-add! bm p)
  (let* ([n ($fxaddress p)]
    ...
    (fxvector-set! a i (fxior (fxvector-ref a i)
                              (fxsll 1 (fxand n (constant eq-bitset-lo-mask)))))))
...
(define (eq-bitset-remove! bm p)
  (let* ([n ($fxaddress p)]
    ...
    (fxvector-set! a i (fxand (fxvector-ref a i)
                              (fxnot (fxsll 1 (fxand n (constant eq-bitset-lo-mask))))))))
