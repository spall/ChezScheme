    ...
    (lambda (total-size leaf-proc merge-proc init)
      (cond
       ...
       [else
        ...
        (let ([cache (make-eq-hashtable)] ; maps st to an assoc list that is keyed by offset+size
              [full-cache (make-eqv-hashtable)] ; maps offset+size to a result
                  ...
                  (cond
                   ...
                   [else
                    (let ([v (do ([size size (fx- size 1)]
                      ...
                      v)]))]
               [else
                ;; We expect `st` values to have distinct addresses for diferent offsets,
                ;; but they don't have to be allocated that way.
